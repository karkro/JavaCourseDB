...
public class JavaLesson05 {
	...
	public static void main(String[] args) {
		...
		// addThem(1, 2) will be replaced with the value that method returns
		...
		System.out.println(getRandomNum()); // Both prints and generates the value of randomNumber
		...
		while (guessResult != -1) {
			...
			// Accepts and integer input from the user
			// You can't declare this variable inside the while loop
			// if you want to access it outside the while loop
			randomGuess = userInput.nextInt();
			guessResult = checkGuess(randomGuess);
		}
		
		System.out.println("Yes, the random number is " + randomGuess);
		System.out.println(randomGuess); // Random value was changed globally by getRandomNum
...
/*	public static int addThem(int a, int b) {
		...
		// compare returns 0 if equal | -1 is smallPI is less than myPI | 1 if smallPI is greater than myPI
		System.out.println(Double.compare(smallPI, myPI));
		
		int c = a + b;
		
		// return a value that replaces the call to this method
		// It must be an int since you defined this method returns into above
