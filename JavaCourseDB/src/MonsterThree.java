...
public class MonsterThree {
	...
	// This static method build an empty battle board
	...
	// You should have as few as possible public class
	...
	// You can create multiple version using the same method name
	...
	public void moveMonster(MonsterThree[] monster, int arrayItemIndex) {
		
		// isSpaceOpen will be used to track whether the space the monster plans to move into is occupied
		...
		// while loop used to make sure I don't move a monster into an occupied space
		while (isSpaceOpen) {
			...
			int randMoveDistance = (int) (Math.random() * (this.getMovements() + 1));
			
			// Prints move distance and move direction
			System.out.println("Direction and distance to move: ");
			System.out.println(nameChar1 + ": " + randMoveDirection + " " + randMoveDistance);
			...
			if (randMoveDirection == 0) {
				...
				// If statement won't allow monster to move off the board
			...
			for (int i = 0; i < monster.length; i++) {
				...
				// if statement skip checking the same monster position against itself
				
				if (i == arrayItemIndex) {
					continue;
				}
				
				// onMySpace receives the monster array, index for the object I'm checking currently,
				// and the index for the monster sent to this function
				
				if (onMySpace(monster, i, arrayItemIndex)) {
					
					// If a monster tries to move to an occupied space 
					// the while loop repeats after I break out of the for loop
	...
	// Check if one monster is trying to move into the same x/y position as another monster
	public boolean onMySpace(MonsterThree[] monster, int indexToChk1, int indexToChk2) {
		...
		if ((monster[indexToChk1].xPosition) == (monster[indexToChk2].xPosition) && (monster[indexToChk1].yPosition) == (monster[indexToChk2].yPosition)) {
			...
			// If the are equal return true so a new x/y position is calculated
			return true;
		} else {
	...
	public MonsterThree(int health, int attack, int movement, String name) {
		this.health = health;
		this.attack = attack;
		...
		// We use a do loop because always want to define a start position for each monster
		do {
		...
		// Adds 1 to the number of monsters on the board
		numOfMonsters++;
